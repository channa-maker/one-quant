...
from sqlalchemy import (
    ...
    BigInteger, ForeignKey, Index, Enum as SAEnum,
)
...
class OrderModel(Base):
    ...
    client_order_id = Column(String(64), unique=True, nullable=False, index=True)  # 客户端订单号（幂等键）
    ...
    status = Column(String(16), nullable=False, default="pending", index=True)  # 状态：pending/filled/cancelled/rejected
...
class AuditLogModel(Base):
    ...
    event_type = Column(String(32), nullable=False, index=True)         # 事件类型：order/risk/strategy/system
...
class LedgerEntryModel(Base):
    ...
    entry_type = Column(String(32), nullable=False)                     # 类型：trade/fee/funding/dividend/deposit/withdrawal
